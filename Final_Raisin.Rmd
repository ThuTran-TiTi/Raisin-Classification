...
PT<-powerTransform(cbind(Area,MajorAxisLength,MinorAxisLength, Perimeter)~1,data=raisin)
summary(PT)
```
...
transform1<-glm(Class ~ I((Area^(-1/5)-1)/(-1/5))+I((MajorAxisLength^(-1/3)-1)/(-1/3)) + I((MinorAxisLength^(-1/3)-1)/(-1/3))+ I((Perimeter^(-1/2)-1)/(-1/2)),
                data = raisin, family = binomial)
summary(transform1)
```
...
library(pROC)
# Prediction
logistic_models <- list(fullmodel,model1,model2,transform1, nullmodel)
Accuracy_score <- c()
...
```
```{r}
models=c('fullmodel','model1','model2','transform1', 'nullmodel')
...
=> Conclusion: 'transform1' has the good value in most of metrics, however the p_value in the summary table shows that the coefficient are not significant. The full model also did a great job in clasification of the raisin model, however the evaluate metric is lower than 'model2'. In conclusion, we choose 'model2' which has 4 predictors (Area, MajorAxisLength, MinorAxisLength, Perimeter) as the final model. 
